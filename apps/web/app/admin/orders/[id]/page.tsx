...
export default async function AdminOrderDetailPage({
  ...
  return (
    <PageShell className="max-w-4xl space-y-4 pb-8">
      {/* ── Header ── */}
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <h1 className="text-xl font-black text-stone-900 sm:text-2xl">{order.orderNumber}</h1>
        ...
        <span
          className={`rounded-full px-3 py-1 text-xs font-semibold ${STATUS_BADGE[order.status] ?? 'bg-stone-100 text-stone-600'}`}
        >
          {STATUS_LABEL[order.status] ?? order.status}
        </span>
      </div>

      {/* ── Mobile quick actions (top, visible only on mobile) ── */}
      <div className="space-y-2 md:hidden">
        <div className="flex gap-2">
          ...
          <Link
            ...
            target="_blank"
            className="flex flex-1 items-center justify-center rounded-xl border border-stone-200 px-4 py-3 text-sm font-medium text-stone-700"
          >
            Receipt
          </Link>
        </div>
        <DeleteOrderButton orderId={order.id} token={token} />
      </div>

      {/* ── Mobile status update (visible only on mobile) ── */}
